...
function Highlight<T extends React.ElementType = "div">({
	...
	const bounds = (props as ParentModeHighlightProps)?.boundsOffset;
	const boundsOffsetTop = bounds?.top ?? 0;
	const boundsOffsetLeft = bounds?.left ?? 0;
	const boundsOffsetWidth = bounds?.width ?? 0;
	const boundsOffsetHeight = bounds?.height ?? 0;
	...
	const safeSetBounds = React.useCallback(
		(bounds: DOMRect) => {
			...
			const newBounds: Bounds = {
				top: bounds.top - containerRect.top + boundsOffsetTop,
				left: bounds.left - containerRect.left + boundsOffsetLeft,
				width: bounds.width + boundsOffsetWidth,
				height: bounds.height + boundsOffsetHeight,
			};
