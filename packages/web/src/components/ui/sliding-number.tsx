import { useMeasure } from "@react-hookz/web";
import {
  type MotionValue,
  motion,
  motionValue,
  type Transition,
  useSpring,
  useTransform,
} from "motion/react";
import { type Ref, useEffect, useId, useRef } from "react";

const TRANSITION: Transition = {
  type: "spring",
  stiffness: 280,
  damping: 18,
  mass: 0.3,
} as const;

function Digit({
  value,
  place,
  animateOnMount,
}: {
  value: number;
  place: number;
  animateOnMount: boolean;
}) {
  const valueRoundedToPlace = Math.floor(value / place) % 10;
  const isFirstRender = useRef(true);
  const initial = motionValue(animateOnMount ? 0 : valueRoundedToPlace);
  const animatedValue = useSpring(initial, TRANSITION);

  useEffect(() => {
    if (isFirstRender.current) {
      isFirstRender.current = false;
      if (animateOnMount) {
        animatedValue.set(valueRoundedToPlace);
      }
      return;
    }
    animatedValue.set(valueRoundedToPlace);
  }, [animatedValue, valueRoundedToPlace, animateOnMount]);

  return (
    <div className="relative inline-block w-[1ch] overflow-x-visible overflow-y-clip leading-none tabular-nums">
      <div className="invisible">0</div>
      {Array.from({ length: 10 }, (_, i) => (
        <NumberElement key={i.toString()} mv={animatedValue} number={i} />
      ))}
    </div>
  );
}

function NumberElement({
  mv,
  number,
}: {
  mv: MotionValue<number>;
  number: number;
}) {
  const uniqueId = useId();
  const [bounds, ref] = useMeasure();

  const y = useTransform(mv, (latest) => {
    if (!bounds?.height) return 0;
    const placeValue = latest % 10;
    const offset = (10 + number - placeValue) % 10;
    let memo = offset * bounds.height;

    if (offset > 5) {
      memo -= 10 * bounds.height;
    }

    return memo;
  });

  // don't render the animated number until we know the height
  if (!bounds?.height) {
    return (
      <span ref={ref as Ref<HTMLSpanElement>} className="invisible absolute">
        {number}
      </span>
    );
  }

  return (
    <motion.span
      style={{ y }}
      layoutId={`${uniqueId}-${number}`}
      className="absolute inset-0 flex items-center justify-center"
      transition={TRANSITION}
      ref={ref as Ref<HTMLSpanElement>}
    >
      {number}
    </motion.span>
  );
}

type SlidingNumberProps = {
  value: number;
  padStart?: boolean;
  decimalSeparator?: string;
  animateOnMount?: boolean;
};

export function SlidingNumber({
  value,
  padStart = false,
  decimalSeparator = ".",
  animateOnMount = false,
}: SlidingNumberProps) {
  const absValue = Math.abs(value);
  const [integerPart, decimalPart] = absValue.toString().split(".");
  const integerValue = parseInt(integerPart, 10);
  const paddedInteger =
    padStart && integerValue < 10 ? `0${integerPart}` : integerPart;
  const integerDigits = paddedInteger.split("");
  const integerPlaces = integerDigits.map(
    (_, i) => 10 ** (integerDigits.length - i - 1),
  );

  return (
    <div className="flex items-center">
      {value < 0 && "-"}
      {integerDigits.map((_, index) => (
        <Digit
          key={`pos-${integerPlaces[index]}`}
          value={integerValue}
          place={integerPlaces[index]}
          animateOnMount={animateOnMount}
        />
      ))}
      {decimalPart && (
        <>
          <span>{decimalSeparator}</span>
          {decimalPart.split("").map((_, index) => (
            <Digit
              key={`decimal-${index.toString()}`}
              value={parseInt(decimalPart, 10)}
              place={10 ** (decimalPart.length - index - 1)}
              animateOnMount={animateOnMount}
            />
          ))}
        </>
      )}
    </div>
  );
}
